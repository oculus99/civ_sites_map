...
def calculate_lakes(
    ...
    precip_annual = np.asarray(precip_annual, dtype=float)
...
def calculate_rivers(relief, precip_annual, pet, cell_size, threshold):
    """
    ...
    netto_vesi = np.maximum(precip_annual - pet, 0.0)
...
if __name__ == "__main__":
    ...
    relief = simulate_thermal_erosion(relief, iterations=5, c_repose=2.0, talus_rate=0.2)
    ...
    #temp_annual = laske_vuosittainen_lampotila(relief, polar_temp, temp_diff, sealevel=0.5, max_korkeus_m=5000)
    #pet = calculate_pet(temp_annual,relief,landmask)
    pet = calculate_pet(
    temps12,
    relief,
    landmask,
    ...
    #plt.contour(precip_annual, levels=[250,500,1000,2000])
    #plt.show()
    #quit(-1)  
    ...
    #current_x, current_y, current_z, upwelling= calculate_ocean_currents_12(
    #dem=dem,
    #wind_x=winds12_x,
    #wind_y=winds12_y,
    #wind_z=winds12_z,
   ...
   # 3. Kutsutaan aluohjelmaa laskentaa varten
    ...
    npp = laske_npp_miami(temp_annual, precip_annual, nodata_arvo=np.nan)
    ...
    #print("Risk shape:", fire_risk.shape)
    #print("Risk min:", np.nanmin(fire_risk))
    #print("Risk max:", np.nanmax(fire_risk))
    #print("Risk mean:", np.nanmean(fire_risk))        
    #plt.imshow(fire_risk[6]*landmask)
    #plt.show() 
    #quit(-1)
    ...
    aly_todennakoisyyskartta = laske_synnyinsija_todennakoisyys(relief, temp_annual, precip_annual)
    ...
    alyevo=laske_evoluutiopaine(relief, temp_annual, precip_annual, vuosisadat=100)
    habitability=laske_human_habitability_index(temp_annual, precip_annual)
    ## mureybat
    target_alt = 250.0
    target_temp = 17
    target_rain = 250.0
    ...
    #target_temp = 25
    #target_rain = 1275
    #cayonu_index,agriculture_index,pastoral_index,suitability,kulttuuri,leviamisaika=etsi_paikka_ja_levia(target_alt, target_temp, target_rain,relief, temp_annual, precip_annual, ajo_vuodet=10000)
    #kulttuuri_index,agriculture_index,pastoral_index,suitability,kulttuuri,leviamisaika=etsi_paikka_ja_levia(target_alt, target_temp, target_rain,relief, temp_annual, precip_annual, ajo_vuodet=10000)
    kulttuuri_tod,agriculture_index,pastoral_index,suitability,kulttuuri,leviamisaika,vaesto,carrying_capacity=etsi_paikka_ja_levia(target_alt, target_temp, target_rain,relief, temp_annual, precip_annual, ajo_vuodet=10000)
    ...
    konduktanssi=np.copy(npp)
    konduktanssi=1/npp
    kantokyky=np.copy(npp)
    ...
    kulttuuri2_tod,agriculture_index2,pastoral_index2,suitability2,kulttuuri2,leviamisaika2,vaesto2,carrying_capacity2=etsi_paikka_ja_levia(target2_alt, target2_temp, target2_rain,relief, temp_annual, precip_annual, ajo_vuodet=1000)
    ...
    konduktanssi2=np.copy(npp)
    konduktanssi2=1/npp
    kantokyky2=np.copy(npp)
    ...
    #plt.show()    
    civ_raster=laske_sivilisaatiopisteet(precip_annual, temp_annual, distrivers, distsea, distmountains, relief)# --- ESIMERKKIKÄYTTÖ ---
    ...
    #plt.imshow(temp_annual, cmap='coolwarm', origin='upper')
    ...
    #im = ax.imshow(precip_annual, cmap='YlGnBu', origin='upper')
    ...
    # with rasterio.open('npp.tif', 'w', **meta) as dst:
    #     dst.write(npp.astype(rasterio.float32), 1)
    # Valinnainen: Tallenna tulos uutena GeoTIFF-tiedostona
