...
#EXTERNAL_DEM_PATH = r"diku1.bmp"
EXTERNAL_DEM_PATH = r"terra.png"
...
MIN_HEIGHT_METERS_UNDER_SEA_LEVEL=-11000
MAX_HEIGHT_METERS=8400 
...
def spherical_noise(
    ...
    """
    Luo pallomaisen 3D Perlin-kohinakartan (Equirectangular).
    Vasen ja oikea reuna ovat täydellisen saumattomat.
    """
    # 1. Luodaan leveys- ja pituusasteiden ruudukot (vektorointi)
    lat = np.linspace(-np.pi / 2, np.pi / 2, height)
    lon = np.linspace(-np.pi, np.pi, width)
    lon_grid, lat_grid = np.meshgrid(lon, lat)
    
    # 2. Muunnetaan pallokoordinaatit 3D-pisteiksi (X, Y, Z)
    nx = scale * np.cos(lat_grid) * np.cos(lon_grid)
    ny = scale * np.cos(lat_grid) * np.sin(lon_grid)
    nz = scale * np.sin(lat_grid)
    
    # 3. Luodaan tyhjä matriisi ja haetaan kohina-arvot
    noise_map = np.zeros((height, width))
    
    # Koska noise-kirjaston pnoise3 ei tue suoraan NumPy-taulukoita syötteenä,
    # käytetään np.vectorize-funktiota ajamaan se tehokkaasti taulukon yli
    v_pnoise3 = np.vectorize(lambda x, y, z: noise.pnoise3(x, y, z, octaves=octaves, 
                                                     persistence=persistence, 
                                                     lacunarity=lacunarity))
    
    noise_map = v_pnoise3(nx, ny, nz)
...
def spherical_sine_offset(
    ...
    """
    Distort an image using a sum of spherical sine waves.
    ...
    # X, Y ja Z oletetaan tässä valmiiksi muodostetuiksi
    # samaan tapaan kuin spherical_noise()-funktiossasi.
    #
    # Niiden koko:
    #
    # (HEIGHT, WIDTH)
    ...
    for wave in waves:
        ...
        # Pallopinnan koordinaatista muodostettu projektio
        coordinate = (
            X * dx
            + Y * dy
        )
    ...
    # ------------------------------------------------------------------
    # Gradientti -> displacement vector
    # ------------------------------------------------------------------
    ...
    # Normalisoidaan gradientti.
    #
    # Tämä estää voimakkaasti muuttuvia alueita tuottamasta
    # suhteettoman suurta displacementia.
...
# ============================================================
# ORGAANINEN / EPÄSÄÄNNÖLLINEN MANNER
# ============================================================

def create_continent_origo(
    center_lon,
    center_lat,
    size
):
    ...
    # --------------------------------------------------------
    # ETÄISYYS MANTEREEN KESKIPISTEESTÄ
    # --------------------------------------------------------
...
def create_continent_testonly(
    ...
    coastal_noise = (
        ...
        * np.sin(
        ...
        # Pienet rannikon epätasaisuudet
        + 0.04
        * np.sin(
            9 * angle
            + phase4
...
def generate_continents():
    ...
    for _ in range(NUM_CONTINENTS):
        ...
        continent = create_continent_origo(
            center_lon,
            center_lat,
            size
        )
        ...
        #continent = create_continent_testonly(
        #    center_lon,
        #    center_lat,
        #    size
        #)
    ...
    noise1 = spherical_noise()   
    land=land*noise1
    land=spherical_noise_offset(land)  
    #land=spherical_sine_offset(land)  
    waves = [
    ...
    #land = spherical_sine_offset(land,waves,strength=12.0,time=4)
...
def add_islands(
    ...
    for _ in range(
        ...
        # ----------------------------------------------------
        # ETÄISYYS SAAREN KESKIPISTEESTÄ
        # ----------------------------------------------------
        ...
        radius_factor = np.sqrt(
            ...
            +
            (
                np.sin(local_angle) / aspect
            ) ** 2
        )

        # ----------------------------------------------------
        # REUNAN EPÄSÄÄNNÖLLISYYS
        # ----------------------------------------------------
...
def erode_terrain_fast_morph(elevation):
    # grey_erosion hakee paikallisen minimin (esim. 3x3 tai 5x5 alueelta)
    # Tämä simuloi materiaalin valumista alaspäin
    eroded = grey_erosion(elevation, size=(3, 3), mode=("nearest", "wrap"))
    
    # Lasketaan eroosioalueet kuten alkuperäisessä koodissasi
    high = np.clip(elevation - 0.55, 0, 1)
    strength = high * 0.4  # Nostettu hieman vaikutusta laaksoissa
    
    return elevation * (1 - strength) + eroded * strength

import numpy as np

def erode_terrain(elevation, iterations=5, c=0.1): ## thermal fast orig 2 iters
    # c = kuinka paljon ainetta valuu per iteraatio (0.05 - 0.2)
    eroded = elevation.copy()
    
    for _ in range(iterations):
        # Lasketaan naapurit (tässä yksinkertainen 4-suuntainen siirros)
        left = np.roll(eroded, -1, axis=1)
        right = np.roll(eroded, 1, axis=1)
        up = np.roll(eroded, -1, axis=0)
        down = np.roll(eroded, 1, axis=0)
        
        # Keskiarvo naapureista edustaa paikkaa, johon aines pyrkii tasoittumaan
        neighbor_avg = (left + right + up + down) * 0.25
        
        # Eroosio tapahtuu vain, jos ollaan naapureita korkeammalla
        diff = eroded - neighbor_avg
        slope_mask = diff > 0.02  # Kynnysarvo: vain jyrkät rinteet reagoivat
        
        # Päivitetään korkeuskarttaa vain maskin alueella
        high = np.clip(eroded - 0.55, 0, 1)
        actual_strength = c * high * slope_mask
        
        eroded -= actual_strength * diff
...
def calculate_temperature(
    ...
    ##MIN_HEIGHT_METERS_UNDER_SEA_LEVEL
    tempcoeff=-6.5*0.0001*((elevation - SEA_LEVEL)*MAX_HEIGHT_METERS)

    temperature_height_effect=elevation*tempcoeff    
    #plt.imshow(temperature_height_effect)
    #plt.show()
    #quit(-1)
    land_temperature=land_temperature+temperature_height_effect 
