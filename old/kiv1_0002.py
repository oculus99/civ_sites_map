...
if __name__ == "__main__":
    ...
    rivers1=calculate_rivers(relief,10,1000)
    ...
    vuoriin=np.where(vuoriin<600,1,0)
    mereen=np.where(mereen<160,1,0)
    sivilisaatio=aavikko*mereen*vuoriin
    ...
    im = plt.imshow(kartta, cmap=cmap_biomit, origin='upper', vmin=0, vmax=7)
    plt.imshow(rivers1*noice, cmap="viridis",origin='upper') 
    civim=plt.imshow(sivilisaatio, cmap="coolwarm_r",origin='upper') 
    ...
    plt.title("Biomes and civilization areas")
    plt.axis('off')
    plt.show()
quit(-1)
