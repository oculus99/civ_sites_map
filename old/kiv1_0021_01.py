...
def laske_planeetan_parametrit(
    planet_mass_me,
    planet_radius_re,
    S1_Wm2,
    star_teff_K,
    ...
    if planet_mass_me <= 0:
        raise ValueError("planet_mass_me pitää olla > 0.")

    if planet_radius_re <= 0:
        raise ValueError("planet_radius_re pitää olla > 0.")

    if S1_Wm2 <= 0:
        raise ValueError("S1_Wm2 pitää olla > 0.")

    if star_teff_K <= 0:
        raise ValueError("star_teff_K pitää olla > 0.")

    if orbital_period_years <= 0:
        raise ValueError(
            "orbital_period_years pitää olla > 0."
        )

    if rotation_period_hours <= 0:
        raise ValueError(
            "rotation_period_hours pitää olla > 0."
        )

    if not 0.0 <= axial_tilt <= 180.0:
        raise ValueError(
            "axial_tilt pitää olla välillä 0...180."
        )
    ...
    if atmosphere_pressure <= 0:
        raise ValueError(
            "atmosphere_pressure pitää olla > 0."
        )

    if not 0.0 < aerosol_coeff <= 1.0:
        raise ValueError(
            "aerosol_coeff pitää olla välillä 0...1."
        )

    if p_co2_ppm <= 0:
        raise ValueError(
            "p_co2_ppm pitää olla > 0."
        )
...
def laske_planeetan_sateily(
    ...
    S1_Wm2 = planeetta["S1_Wm2"]
    aerosol_coeff = planeetta["aerosol_coeff"]
    p_co2_ppm = planeetta["p_co2_ppm"]
    atmosphere_pressure = planeetta["atmosphere_pressure"]

    if not 0.0 <= albedo_keski < 1.0:
        raise ValueError(
            "albedo_keski pitää olla välillä 0...1."
        )
    ...
    return {
        ...
        "aerosolien_jalkeen_Wm2":
            transmitted_Wm2,

        "absorboituva_sateily_Wm2":
            absorbed_Wm2,

        "globaali_absorboituva_sateily_Wm2":
            absorbed_global_Wm2,

        "nolla_atmosfaari_sateily_Wm2":
            absorbed_global_Wm2,

        "nolla_atmosfaari_lampotila_K":
            T_effective_K,

        "nolla_atmosfaari_lampotila_C":
            T_effective_C,

        "ilmakeha_lampotila_K":
            T_atmosphere_K,

        "ilmakeha_lampotila_C":
            T_atmosphere_C,

        "ulosmeneva_pitkaaaltoinen_sateily_Wm2":
            outgoing_lw_Wm2,
...
# ============================================================
# 3. PLANEETAN GLOBAALI LÄMPÖTILA
# ============================================================

def laske_planeetan_lampotila2(
    planeetta,
    albedo_keski=0.30,
    vesihoyry_kerroin=1.0,
    kasvihuone_kerroin=1.0,
    ilmastosensitiivisyys=0.8,
    lammon_kuljetus_paivantaasaajalta_navoille=0.5,
    lampo_kapasiteetti=1.0,
    vuodenaika_kerroin=1.0,
):

    atmosphere_pressure = (
        planeetta["atmosphere_pressure"]
    )

    rotation_period_days = (
        planeetta["rotation_period_days"]
    )

    planet_radius_re = (
        planeetta["planet_radius_re"]
    )

    gravity_relative = (
        planeetta["gravity_relative"]
    )

    ecc = planeetta["ecc"]
    tilt = planeetta["axial_tilt"]
    mvelp = planeetta["mvelp"]

    sateily = laske_planeetan_sateily(
        planeetta=planeetta,
        albedo_keski=albedo_keski,
        vesihoyry_kerroin=vesihoyry_kerroin,
        kasvihuone_kerroin=kasvihuone_kerroin,
        ilmastosensitiivisyys=ilmastosensitiivisyys,
    )

    T_effective_C = (
        sateily["nolla_atmosfaari_lampotila_C"]
    )
    ...
    T_global_C = (
        sateily["ilmakeha_lampotila_C"]
        +
        rotation_delta_T_C
    )

    # Maan kalibrointi.
    # 280 ppm CO2 + 1 atm -> noin 15 C.

    T_global_C += 0.0

    # ========================================================
    # ALUEELLINEN AURINKOENERGIA
    # ========================================================
    ...
    # ========================================================
    # ALUEELLINEN TEHOLLINEN LÄMPÖTILA
    # ========================================================
    ...
    return {
        ...
        "planet_mass_me":
            planeetta["planet_mass_me"],

        "planet_radius_re":
            planet_radius_re,

        "planet_radius_km":
            planeetta["planet_radius_km"],

        "gravity_relative":
            gravity_relative,

        "gravity_ms2":
            planeetta["gravity_ms2"],

        "rotation_period_hours":
            planeetta["rotation_period_hours"],

        "rotation_period_days":
            rotation_period_days,

        "kiertosolut":
            planeetta["kiertosolut"],

        "S1_Wm2":
            sateily["S1_Wm2"],

        "aerosolien_jalkeen_Wm2":
            sateily["aerosolien_jalkeen_Wm2"],

        "absorboituva_sateily_Wm2":
            sateily["absorboituva_sateily_Wm2"],

        "globaali_absorboituva_sateily_Wm2":
            sateily["globaali_absorboituva_sateily_Wm2"],

        "nolla_atmosfaari_lampotila_C":
            sateily["nolla_atmosfaari_lampotila_C"],

        "nolla_atmosfaari_lampotila_K":
            sateily["nolla_atmosfaari_lampotila_K"],
        ...
        "nettosateily_Wm2":
            sateily["nettosateily_Wm2"],

        "kasvihuone_delta_T_C":
            sateily["kasvihuone_delta_T_C"],

        "co2_ppm":
            planeetta["p_co2_ppm"],

        "co2_sateilypakote_Wm2":
            sateily["co2_sateilypakote_Wm2"],

        "co2_delta_T_C":
            sateily["co2_delta_T_C"],
...
def laske_planeetan_lampotila(
    ...
    """
    Laskee planeetan globaalin lämpötilan sekä
    päiväntasaajan ja napojen lämpötilaeron.

    Tätä funktiota käytetään myöhemmin myös
    kuukausittaisen ilmaston lähtöarvona.
    """
    ...
    # ========================================================
    # TEHOLLINEN LÄMPÖTILA
    # ========================================================

    def vuo_to_effective_temp(
        vuo,
        geometria,
    ):
...
def laske_perus_ilmasto(
    ...
    """
    Laskee planeetan 12 kuukauden ilmaston.

    Uusi versio ottaa huomioon:

        - meren määrän
        - etäisyyden merestä
        - mereltä tulevan tuulen
        - lämpimän maan määrän
        - lämpimän maan haihdunnan
        - maankosteuden
        - orografian
        - vuodenaikojen vaihtelun
        - lämpötilan vaikutuksen sadantaan

    Erityisesti lämmin maa vaikuttaa nyt kosteuden
    kierrätykseen. Näin planeetta, jolla suuri osa
    maasta sijaitsee tropiikissa tai lämpimillä
    leveysasteilla, ei käyttäydy samalla tavalla kuin
    planeetta, jolla sama maapinta-ala sijaitsee
    kylmillä leveysasteilla.
    """
    ...
    orbital_period_years = (
        planeetta["orbital_period_years"]
    )
    ...
    if mean_temp is None:
        ...
        ilmasto = laske_planeetan_lampotila(
            planeetta
        )
    ...
    # PINTA-ALAN PAINOT
    #
    # Tasavälisellä leveysasteverkolla soluilla ei ole
    # sama pinta-ala.
    #
    # cos(latitude) korjaa tämän.
    # ========================================================
    ...
    # ========================================================
    # KOSTEUS / MEREN LÄMPÖMUISTI
    # ========================================================

    maankosteus = np.where(
        landmask,
        0.45,
        1.0
    ).astype(float)

    maankosteus[seamask] = 1.0
    ...
    for kk in range(12):
        ...
        # ====================================================
        # LÄMPIMÄN MAAN INDEKSI
        #
        # 0 = kylmä
        # 1 = hyvin lämmin
        #
        # Tätä käytetään sekä haihduntaan että
        # sademäärän paikalliseen kierrätykseen.
        # ====================================================
        ...
        # ----------------------------------------------------
        # Lämmin maa haihduttaa tehokkaammin.
        #
        # Kuiva maa ei kuitenkaan saa valtavaa haihduntaa,
        # koska maankosteus rajoittaa sitä.
        # ----------------------------------------------------

        lämmin_maa_haihdunta = (
            1.0
            +
            0.35 *
            lämmin_maa
        ...
        # ====================================================
        # LÄMPIMÄN MAAN SADEKIERRÄTYS
        #
        # Lämmin ja kostea maa voi palauttaa vettä
        # ilmakehään haihdunnan ja kasvillisuuden kautta.
        #
        # Kylmä maa saa vain pienen vaikutuksen.
        # ====================================================
    ...
    # ========================================================
    # LOPULLINEN LÄMPÖTILAN KALIBROINTI
    # ========================================================
    ...
    # LÄMPIMÄN MAAN MÄÄRÄ
    #
    # Nyt ratkaistaan kuinka paljon koko planeetan
    # pinta-alasta on lämmintä maata.
    #
    # Tämä on tärkeä uusi planeettatason muuttuja.
    # ========================================================
    ...
    lämmin_maa_vuositasolla = np.clip(
        (
            temp_annual -
            5.0
        ) / 25.0,
    ...
    lämmin_maa_vuositasolla *= (
        land
    )

    # ========================================================
    # PLANEETAN MERI- JA MAAOSUUDET
    # ========================================================
    ...
    # ========================================================
    # LÄMPIMÄN MAAN OSUUS KOKO PLANEETASTA
    # ========================================================
    ...
    # ========================================================
    # LÄMPIMÄN MAAN OSUUS MAAN SISÄLLÄ
    # ========================================================

    if maa_osuus > 1e-8:

        lämmin_maa_osuus_maasta = (
            lämmin_maa_osuus /
            maa_osuus
        )
        ...
        lämmin_maa_osuus_maasta = 0.0

    # ========================================================
    # PLANEETAN KOSTEUSLÄHTEEN KOKONAISINDEKSI
    #
    # Earth-tyyppinen kalibrointi:
    #
    # meri noin 71 %
    # lämmin maa noin 12 % planeetan pinta-alasta
    #
    # Meri saa suurimman painon.
    # Lämmin maa vaikuttaa kuitenkin selvästi.
    # ========================================================

    earth_sea_reference = 0.71

    earth_warm_land_reference = 0.12
    ...
    lämmin_maa_kerroin = (
        lämmin_maa_osuus /
        earth_warm_land_reference
    )

    kosteuden_lahde_indeksi = (
        0.60 *
        meri_kerroin
        +
        0.25 *
    ...
    # ========================================================
    # GLOBAALIN LÄMPÖTILAN VAIKUTUS
    #
    # Lämpimämpi ilmasto pystyy käsittelemään enemmän
    # vesihöyryä.
    #
    # Tämä pidetään maltillisena.
    # ========================================================

    lampo_kosteus_kerroin = (
        math.exp(
            0.018 *
            (
                mean_temp -
    ...
    # ========================================================
    # MAAILMANLAJUINEN SADEKALIBROINTI
    #
    # Nykyinen sadealgoritmi tuottaa muuten liian pienen
    # absoluuttisen sadannan.
    #
    # Tämä kerroin kalibroi Maan suuruusluokan noin
    # 1000 mm/vuosi ympärille.
    # ========================================================

    earth_precipitation_scale = 7.0

    globaali_sade_kerroin = (
        earth_precipitation_scale
        *
        kosteuden_lahde_indeksi
        *
        lampo_kosteus_kerroin
...
def laske_planeetan_ilmasto(
    ...
    """
    Koko planeetan ilmaston pääfunktio.

    Sademäärässä huomioidaan:

        - meren kokonaisosuus
        - lämpimän maa-alan määrä
        - erittäin lämpimän maa-alan määrä
        - lämpötilan vaikutus haihduntaan
        - meren kosteuden saatavuus
        - maa/meri-yhdistelmä

    Paikallinen ilmastomalli vastaa edelleen:
    ...
    planeetta = laske_planeetan_parametrit(
    ...
    painom_matriisi = np.repeat(
        painot_1d[:, np.newaxis],
        width,
        axis=1
    )
    ...
    tulokset = (
        calculate_land_sea_percentage(
            landmask
        )
    )

    mannerosuus = (
        tulokset["maa_prosentti"]
        /
        100.0
    )

    meriosuus = (
        tulokset["meri_prosentti"]
        /
        100.0
    )
    ...
    manner_efekti = (
        laske_lammonkuljetus(
            mannerosuus
        )
    )

    # ========================================================
    # PLANETAAN KESKILÄMPÖTILA
    # ========================================================

    alueen_keski_lampotila = np.average(
        temp_annual,
        weights=painom_matriisi
    )

    # ========================================================
    # LÄMPIMÄN MAA-ALAN TUNNISTUS
    #
    # Tämä on tärkeä muutos.
    #
    # Pelkkä maan määrä ei ratkaise sadetta.
    #
    # Esimerkiksi:
    #
    # 30 % maata tropiikissa
    #
    # on sateen kannalta aivan eri asia kuin
    #
    # 30 % maata napa-alueilla.
    # ========================================================
    ...
    # ========================================================
    # LÄMPIMÄN MAA-ALAN OSUUS KOKO PLANEETASTA
    # ========================================================

    warm_land_fraction = np.average(
        warm_land_mask.astype(float),
        weights=painom_matriisi
    )

    hot_land_fraction = np.average(
        hot_land_mask.astype(float),
        weights=painom_matriisi
    )

    very_hot_land_fraction = np.average(
        very_hot_land_mask.astype(float),
        weights=painom_matriisi
    )

    # ========================================================
    # LÄMPIMÄN MAA-ALAN OSUUS KAIKESTA MAASTA
    # ========================================================
    ...
    if mannerosuus > 1e-8:
        ...
        warm_land_of_land = 0.0
        hot_land_of_land = 0.0

    warm_land_of_land = np.clip(
        warm_land_of_land,
        0.0,
        1.0
    )

    hot_land_of_land = np.clip(
        hot_land_of_land,
        0.0,
        1.0
    )
    ...
    # LÄMPÖTILAN VAIKUTUS HAIHDUNTAAN
    #
    # Lämmin planeetta -> enemmän haihduntaa
    #
    # Mutta vaikutus pidetään rajattuna.
    # ========================================================

    lampo_haihdunta_kerroin = np.clip(
        (
            alueen_keski_lampotila
            +
            5.0
        )
        /
    ...
    # ========================================================
    # MEREN LÄMPÖTILA / KOSTEUSPOTENTIAALI
    #
    # Kylmä meri antaa vähemmän kosteutta.
    # Lämmin meri antaa enemmän.
    # ========================================================

    meri_lampo_kerroin = np.clip(
        (
            alueen_keski_lampotila
            +
            5.0
    ...
    # ========================================================
    # MEREN KOSTEUSPOTENTIAALI
    # ========================================================

    meri_kosteus_potentiaali = (
        meriosuus
        *
        meri_lampo_kerroin
    )
    ...
    # LÄMMIN MAA + MERI
    #
    # Sateen kannalta tärkeä yhdistelmä:
    #
    # lämmin maa
    #
    # +
    ...
    # lämpötila
    #
    # Jos lämmintä maata on paljon mutta meri puuttuu,
    # planeetta kuivuu helpommin.
    # ========================================================
    ...
    # ========================================================
    # TROOPPISEN / ERITTÄIN LÄMPIMÄN MAA-ALAN LISÄVAIKUTUS
    #
    # >25 C alueet lisäävät haihduntaa ja kosteuden kiertoa.
    # ========================================================
    ...
    #
    # Suuri meriosuus:
    #     kosteuden saatavuus kasvaa
    #
    # Mutta pelkkä meri ei riitä.
    # Tarvitaan myös lämmintä maa-alaa.
    # ========================================================
    ...
    # ========================================================
    # KOKONAISEN SADEVEDEN KOSTEUSPOTENTIAALI
    # ========================================================

    globaali_sadekerroin = (
        maa_meri_kerroin
        *
        lämmin_maa_kerroin
        *
        kuuma_maa_kerroin
        *
        lampo_haihdunta_kerroin
    )

    # ========================================================
    # KALIBROINTI
    #
    # Maan pitäisi saada järkevä vuosikeskiarvo.
    #
    # Kerroin pidetään kohtuullisella alueella, jotta
    # äärimmäiset planeetat eivät räjähdä sateen suhteen.
    # ========================================================

    globaali_sadekerroin = np.clip(
        globaali_sadekerroin,
        0.50,
        4.0
    )
    ...
    # LOPULLINEN SADE
    #
    # Paikallinen sadejakauma säilyy.
    #
    # Tämä kerroin muuttaa ensisijaisesti planeetan
    # kokonaiskosteuden määrää.
    # ========================================================
    ...
    # ========================================================
    # LOPULLISET KESKIARVOT
    # ========================================================

    alueen_keski_sademaara = np.average(
        precip_annual,
        weights=painom_matriisi
    )
    ...
    print(
        f"Mass              : "
        f"{planet_mass_me:.3f} M_E"
    )

    print(
        f"Radius            : "
        f"{planet_radius_re:.3f} R_E"
    )

    print(
        f"Radius            : "
        f"{planeetta['planet_radius_km']:.1f} km"
    )

    print(
        f"Gravity           : "
        f"{planeetta['gravity_ms2']:.2f} m/s²"
    )
    ...
    print(
        f"Orbital period    : "
        f"{orbital_period_years:.3f} years"
    )

    print(
        f"Rotation period   : "
        f"{rotation_period_hours:.2f} h"
    )
    ...
    print(
        f"Stellar radiation : "
        f"{ilmasto['S1_Wm2']:.2f} W/m²"
    )

    print(
        f"After aerosols    : "
        f"{ilmasto['aerosolien_jalkeen_Wm2']:.2f} W/m²"
    )

    print(
        f"Absorbed           : "
        f"{ilmasto['globaali_absorboituva_sateily_Wm2']:.2f} W/m²"
    )

    print(
        f"No atmosphere T   : "
        f"{ilmasto['nolla_atmosfaari_lampotila_C']:.2f} °C"
    )

    print(
        f"Atmosphere T      : "
        f"{ilmasto['ilmakeha_lampotila_C']:.2f} °C"
    )

    print(
        f"Greenhouse ΔT     : "
        f"{ilmasto['kasvihuone_delta_T_C']:.2f} °C"
    )

    print(
        f"CO2 forcing       : "
        f"{ilmasto['co2_sateilypakote_Wm2']:.2f} W/m²"
    )

    print(
        f"CO2 ΔT            : "
        f"{ilmasto['co2_delta_T_C']:.2f} °C"
    )

    print(
        f"Rotation ΔT       : "
        f"{ilmasto['pyorimisen_delta_T_C']:.2f} °C"
    )

    print(
        f"Global T          : "
        f"{ilmasto['globaali_lampotila_C']:.2f} °C"
    )
    ...
    print(
        f"Land              : "
        f"{tulokset['maa_prosentti']:.2f}%"
    )

    print(
        f"Sea               : "
        f"{tulokset['meri_prosentti']:.2f}%"
    )

    print(
        f"Warm land         : "
        f"{warm_land_fraction * 100.0:.2f}%"
    )

    print(
        f"Hot land          : "
        f"{hot_land_fraction * 100.0:.2f}%"
    )

    print(
        f"Warm land of land : "
        f"{warm_land_of_land * 100.0:.2f}%"
    )

    print(
        f"Rainfall factor   : "
        f"{globaali_sadekerroin:.3f}"
    )

    print(
        f"Deepest sea       : "
        f"{dem_stats['min_sea_depth_m']:.1f} m"
    )

    print(
        f"Highest peak      : "
        f"{dem_stats['max_land_height_m']:.1f} m"
    )

    print(
        f"Ocean mean depth  : "
        f"{dem_stats['mean_sea_depth_m']:.1f} m"
    )

    print(
        f"Land mean height  : "
        f"{dem_stats['mean_land_height_m']:.1f} m"
    )
    ...
    print(
        f"Mean surface T    : "
        f"{alueen_keski_lampotila:.2f} °C"
    )

    print(
        f"Mean precipitation: "
        f"{alueen_keski_sademaara:.1f} mm/year"
    )
...
def fractal_height_modifier00(ax, p=2.5, a=3.0, lam=0.3):
    """
    Muokkaa normalisoitua korkeutta ax [0,1]
    fraktaalimaisen / luonnollisen korkeusjakauman suuntaan.

    p   = fraktaalijakauman muoto
    a   = eksponentiaalisen komponentin voimakkuus
    lam = power- ja exponential-komponenttien sekoitus
    """

    ax = max(0.0, min(1.0, ax))

    # Fraktaalinen power-law -muunnos
    power = ax ** p

    # Eksponentiaalinen komponentti, normalisoituna [0,1]
    exponential = (
        math.exp(a * ax) - 1.0
    ) / (
        math.exp(a) - 1.0
    ...
    # Yhdistelmä
    dem = (1.0 - lam) * power + lam * exponential

    return max(0.0, min(1.0, dem))


def fractal_height_modifier(ax, p=2.5, a=3.0, lam=0.3):
    """
    Fraktaalinen korkeusjakauman muokkaaja.

    ax  : NumPy-array, normalisoitu korkeuteen [0, 1]
    p   : power-law / fraktaalidimension muoto
    a   : eksponentiaalisen komponentin voimakkuus
    lam : power-law- ja eksponentiaalisen komponentin sekoitus
    """

    # Varmistetaan alue [0, 1]
    ax = np.clip(ax, 0.0, 1.0)

    # Fraktaalinen power-law komponentti
    fractal = np.power(ax, p)

    # Eksponentiaalinen komponentti, normalisoituna [0, 1]
    exponential = np.expm1(a * ax) / np.expm1(a)

    # Yhdistetään komponentit
    h = (1.0 - lam) * fractal + lam * exponential

    # Palautetaan jakauman kontrastia hieman
    h = np.power(h, 1.0 / p)
...
def luokittele_biomit(dem, temp_annual, precip_annual, sealevel=0.5):
    """
...
def global_pixel_areas(H, W, planet_radius_mm):
    """
    Palauttaa pikselien pinta-alat mm² muodossa (H, W).
    Oletetaan säännöllinen global lon/lat -rasteri.
    """

    dlon = 2.0 * np.pi / W

    # Latitudien pikselireunat
    lat_edges = np.linspace(-np.pi / 2, np.pi / 2, H + 1)

    # Pallon pinnan ala:
    # A = R² * dlon * (sin(lat_north) - sin(lat_south))
    row_area = (
        planet_radius_mm ** 2
        * dlon
        * (
    ...
    return np.broadcast_to(row_area[:, None], (H, W)).copy()

def thornthwaite_pet(temps12):
    """
    Hyvin yksinkertaistettu Thornthwaite PET.
    temps12: (12,H,W), Celsius

    Palauttaa vuosittaisen PET:n mm/vuosi.
    """

    T = np.maximum(temps12, 0.0)

    # kuukausittainen lämpöindeksi
    i = (T / 5.0) ** 1.514

    annual_heat_index = np.sum(i, axis=0)
    ...
    # Thornthwaiten kuukausi-PET
    pet = np.zeros_like(T)

    valid = annual_heat_index > 0

    for m in range(12):
        pet[m][valid] = (
            16.0
            * (10.0 * T[m][valid] / annual_heat_index[valid]) ** alpha[valid]
        )

    return np.sum(pet, axis=0)

def gaussian_score(x, optimum, sigma):
...
def calculate_pixel_area(
    ...
    """
    Pallon lon/lat-rasterin tarkat pikselialat.
...
def calculate_terrain(
    ...
    *,
    ...
    # --------------------------------------------------------
    # Slope on spherical lon/lat grid
    # --------------------------------------------------------
...
N_AVOGADRO = 6.02214076e23       # photons/mol
...
def stellar_par_properties(
    star_teff,
    wavelength_min_nm=400.0,
    wavelength_max_nm=700.0,
    wavelength_max_total_nm=10000.0,
    n=30000,
):
    """
    Blackbody approximation for stellar radiation.

    Returns
    -------
    energy_fraction
        Fraction of stellar energy in PAR.

    photon_fraction
        Fraction of stellar photons in PAR.

    par_mean_wavelength_nm
        Photon-weighted mean PAR wavelength.
    """

    wl = np.linspace(
        1e-9,
        wavelength_max_total_nm * 1e-9,
        n,
    )

    x = (
        H_PLANCK * C_LIGHT
        / (wl * K_BOLTZMANN * star_teff)
    )

    B = (
        2.0 * H_PLANCK * C_LIGHT**2
        / (
            wl**5
            * np.expm1(
                np.minimum(x, 700.0)
    ...
    total_photons = np.trapezoid(
        photon_spectrum,
        wl,
    )

    par_mask = (
        (wl >= wavelength_min_nm * 1e-9)
        & (wl <= wavelength_max_nm * 1e-9)
    )

    par_energy = np.trapezoid(
        B[par_mask],
        wl[par_mask],
    )

    par_photons = np.trapezoid(
        photon_spectrum[par_mask],
        wl[par_mask],
    )

    par_wavelength = np.trapezoid(
        (
            wl[par_mask]
            * photon_spectrum[par_mask]
        ),
    ...
    return {
        "energy_fraction":
            par_energy / total_energy,

        "photon_fraction":
            par_photons / total_photons,
...
def solve_kepler(
    ...
    """
    Vectorized Newton solver for Kepler's equation:
...
def orbital_position(
    ...
    """
    Returns orbital distance and true anomaly.

    Distance is normalized to semi-major axis.

    mvelp defines perihelion phase.
    """
    ...
    # Solar longitude measured from reference direction
    solar_longitude = (
        true_anomaly
        + mvelp
    )
...
# ============================================================
# 3. PLANETARY PIXEL GEOMETRY
# ============================================================

def latitude_grid(H):
    """
    Pixel-center latitude grid.
    """
...
# ============================================================
# 4. HOURLY SOLAR GEOMETRY
# ============================================================

def hourly_solar_radiation(
    latitudes,
    solar_declination,
    solar_flux,
    hour_angle,
    ...
    """
    Instantaneous TOA solar radiation.
...
def calculate_hourly_PAR(
    ...
    *,
    ...
    mvelp,
    orbital_period_in_years,
    rotation_period_in_hours,
    days_per_year=365,
    hours_per_rotation=None,
):
    """
    Full hourly TOA -> PAR model.
    ...
    Notes
    -----
    rotation_period_in_hours changes the solar hour angle.

    The grid itself is longitude-independent for the monthly
    global climatology. Longitude can be introduced later if
    an absolute time-of-day field is required.
    """

    if hours_per_rotation is None:
        hours_per_rotation = (
            rotation_period_in_hours
        )
    ...
    # --------------------------------------------------------
    #
    # We use one local solar day as the rotation period.
    #
    # Solar hour angle:
    #
    #   H = 2*pi * fraction_of_rotation
    #
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # PAR spectral fraction
    # --------------------------------------------------------

    spectral = stellar_par_properties(
        star_teff
    )

    par_energy_fraction = (
        spectral["energy_fraction"]
    )
    ...
    PAR12 = np.broadcast_to(
        PAR12_lat[:, :, None],
        (12, H, W),
    ).copy()

    SW12 = np.broadcast_to(
        SW12_lat[:, :, None],
        (12, H, W),
    ).copy()
...
def thornthwaite_pet(
    ...
    This is a simple climatic PET estimate.
    """
...
def water_balance(
    ...
    """
    Simple monthly soil-water model.

    This is intentionally more realistic than simply
    min(PET, P): it allows unused precipitation to remain
    temporarily as soil water.

    Soil storage capacity is estimated from precipitation.
    """
    ...
    # Dynamic soil water capacity
    soil_capacity = np.clip(
        50.0
        + 0.25 * annual_P,
        50.0,
        300.0,
...
def temperature_response(
    ...
    """
    Broad photosynthetic temperature response.
...
# ============================================================
# 9. ANPP FROM PAR + WATER + TEMPERATURE
# ============================================================
...
def calculate_anpp(
    ...
    *,
    ...
    """
    Converts PAR to annual dry biomass.

    PAR:
        mol photons/m²/day

    epsilon:
        kg dry matter / mol absorbed PAR

    The model uses monthly environmental limitations.
    ...
    # Growing-season precipitation response
    P_factor = np.clip(
        precips12 / 50.0,
        0.0,
        1.0,
    ...
    return {
        ...
        "temperature_factor12":
            T_factor,

        "water_factor12":
            water_factor,

        "productivity_limitation12":
            limitation,
    }
...
# ============================================================
# 11. PASTORAL NOMAD INDEX
# ============================================================

def pastoral_nomad_index(
    annual_precip,
    mean_temp,
    aridity_index,
    water_stress,
...
# ============================================================
# 12. LIVESTOCK CARRYING CAPACITY
# ============================================================

def livestock_capacity(
    usable_forage_kg_ha,
    pixel_area_ha,
    pixel_area_km2,
    *,
    dm_per_lsu_year=3650.0,
    pastoral_land_fraction=0.70,
):
    """
    Livestock capacity.

    Returns camel, sheep/goat and cattle capacity.
    """
    ...
    for animal, params in animals.items():
        ...
        population = (
            lsu_per_pixel
            * pastoral_land_fraction
            / params["lsu_per_person"]
        )
...
def calculate_pastoral_nomadism(
    precips12,
    temps12,
    relief,
    planet_radius,
    *,
    ...
    """
    COMPLETE GLOBAL PASTORAL NOMADISM MODEL.
    ...
    temps12:
        ...
        NOTE:
        temperature is assumed already elevation-corrected.
    ...
    mvelp:
        perihelion phase [deg]

    orbital_period_in_years:
        orbital period

    rotation_period_in_hours:
        sidereal/local rotation period [h]

    Returns
    -------

    dict containing all major rasters.
    """
    #relief isn in mm, planet radius is in mm
    P = np.asarray(
        precips12,
        dtype=np.float64,
    )
    ...
    if P.ndim != 3:
        raise ValueError(
            "precips12 pitää olla (12,H,W)"
        )

    if T.shape != P.shape:
        raise ValueError(
            "temps12 ja precips12 eivät täsmää"
        )

    if P.shape[0] != 12:
        raise ValueError(
            "Rasterin ensimmäinen dimensio pitää olla 12"
        )
    ...
    PNI = pastoral_nomad_index(
        annual_precip,
        mean_temp,
        aridity_index,
        water["water_stress"],
        growing_months,
        terrain["elevation_score"],
        terrain["slope_score"],
    )
    ...
    usable_forage_kg_ha = masked(
        usable_forage_kg_ha
    )

    annual_precip = masked(
        annual_precip
    )
    ...
    for animal in livestock:
        ...
        for key in livestock[animal]:

            livestock[animal][key] = masked(
                livestock[animal][key]
            )
    ...
    result = {
        ...
        "par_energy_fraction":
            solar["par_energy_fraction"],

        "par_photon_fraction":
            solar["par_photon_fraction"],
    ...
    for animal in livestock:
        ...
        result[
            f"{animal}_lsu_per_ha"
        ] = livestock[animal][
            "lsu_per_ha"
        ]

        result[
            f"{animal}_lsu_per_pixel"
        ] = livestock[animal][
            "lsu_per_pixel"
        ]

        result[
            f"{animal}_population"
        ] = livestock[animal][
            "population"
        ]
...
def calculate_et0_penman_monteith(
    ...
    """
    Approximate monthly ET0.

    temps12:      °C
    radiation12:  W/m², monthly mean
    wind12:       m/s
    humidity12:   relative humidity, %
    
    Returns:
        ET0 in mm/month
    """

    t = np.asarray(temps12, dtype=float)
    rs = np.asarray(radiation12, dtype=float)
    u2 = np.asarray(wind12, dtype=float)
    rh = np.asarray(humidity12, dtype=float)

    # Prevent physically problematic values
    u2 = np.maximum(u2, 0.0)
    rh = np.clip(rh, 0.0, 100.0)

    # Radiation
    rs_mj = radiation_to_mj_m2_day(rs)
    ...
    # Approximation: actual vapour pressure
    ea = es * rh / 100.0
    ...
    # Soil heat flux.
    # For monthly time scale this simplified version
    # assumes G ≈ 0.
    g = 0.0

    # FAO-56 style reference ET.
    et0_daily = (
        (
            0.408 * delta * (rs_mj - g)
            +
...
def calculate_et0(
    ...
    if (
        radiation12 is not None
        and wind12 is not None
        and humidity12 is not None
    ):
        return calculate_et0_penman_monteith(
            temps12,
            radiation12,
            wind12,
            humidity12,
    ...
    return calculate_et0_temperature(
        temps12,
    )
...
def radiation_to_mj_m2_day(radiation12):
    """
    W/m² monthly mean -> MJ/m²/day
    """
    return radiation12 * 0.0864
...
def calculate_irrigation_need(
    ...
    et0 = calculate_et0(
        temps12,
        radiation12=radiation12,
        wind12=wind12,
        humidity12=humidity12,
    )

    deficit12 = calculate_water_deficit(
        et0,
        precips12,
    )

    irrigation_need = np.sum(
        deficit12,
        axis=0,
    )
...
def calculate_water_deficit(et0, precips12):
    return np.maximum(
        et0 - precips12,
        0.0,
    )
...
def luokittele_holdridge(
    lampotila_matriisi,
    sade_matriisi,
    nodata_arvo=None,
):
    """
    Luokittelee rasterisolut viiteen Holdridge-tyyppiseen vyöhykkeeseen.

    Parametrit
    ----------
    lampotila_matriisi : numpy.ndarray
        Vuoden keskilämpötila (°C).
    sade_matriisi : numpy.ndarray
        Vuotuinen sademäärä (mm).
    nodata_arvo : float, optional
        Alkuperäinen NoData-arvo. NoData palautetaan arvona -1.

    Palauttaa
    ----------
    numpy.ndarray
        Kokonaislukumatriisi, jossa:
            -1 = NoData
             1 = Aavikko
             2 = Aro / steppe
             3 = Kuiva metsä
             4 = Kostea metsä / sademetsä
             5 = Tundra / alpiininen
    """

    # Muutetaan floatiksi, jotta NaN voidaan käyttää NoData-arvona.
    t = np.asarray(lampotila_matriisi, dtype=float).copy()
    p = np.asarray(sade_matriisi, dtype=float).copy()

    if t.shape != p.shape:
        raise ValueError(
            "Lämpötila- ja sadematriisien täytyy olla samanmuotoisia."
        )
    ...
    # Myös mahdolliset jo olemassa olevat NaN-arvot säilyvät.
    nodata_maski = np.isnan(t) | np.isnan(p)

    # Biolämpötilan approksimaatio vuosikeskilämpötilasta.
    #
    # HUOM:
    # Aidossa Holdridge-menetelmässä biolämpötila lasketaan
    # kuukausilämpötiloista. Tämä on vuosikeskilämpötilaan
    # perustuva approksimaatio.
    biolampotila = np.clip(t, 0, 30)

    # Alustetaan NoData-arvoksi -1.
    luokat = np.full(t.shape, -1, dtype=np.int8)

    # Vain validit pikselit luokitellaan.
    validi = ~nodata_maski

    # Tundra / alpiininen
    maski = validi & (biolampotila < 1.5)
    luokat[maski] = 5

    # Aavikko
    maski = validi & (biolampotila >= 1.5) & (p < 250)
    luokat[maski] = 1
    ...
    # Kostea metsä / sademetsä
    maski = validi & (biolampotila >= 1.5) & (p >= 1000)
    luokat[maski] = 4
...
def piirra_holdridge_vyohykkeet(
    luokat,
    ax=None,
    otsikko="Holdridgen elämänvyöhykkeet",
):
    """
    Piirtää luokitellun Holdridge-rasterin.

    Parametrit
    ----------
    luokat : numpy.ndarray
        luokittele_holdridge()-funktion palauttama matriisi.
        Luokat 1-5, NoData = -1.
    ax : matplotlib.axes.Axes, optional
        Olemassa oleva akseli.
    otsikko : str
        Kartan otsikko.
    ...
    # Värit luokille 1-5.
    # Indeksi 0 vastaa NoDataa, indeksit 1-5 luokkia.
    varit = [
        "white",       # NoData
        "#e6c229",     # 1 Aavikko
        "#d98c3f",     # 2 Steppe
        "#8abf69",     # 3 Kuiva metsä
        "#237a3b",     # 4 Kostea metsä
        "#b9d7ea",     # 5 Tundra / alpiininen
    ]

    cmap = ListedColormap(varit)

    # -1, 1, 2, 3, 4, 5
    # Muutetaan -1 indeksiksi 0 ja luokat 1-5 vastaavasti
    piirrettava = luokat.copy()
    piirrettava[piirrettava == -1] = 0
    ...
    # Erillinen väripalkki kategorioille
    cbar = plt.colorbar(
        kuva,
        ax=ax,
        ticks=range(6),
    ...
    cbar.ax.set_yticklabels(nimet)

    ax.set_title(otsikko)
    ax.set_xlabel("Sarake")
    ax.set_ylabel("Rivi")
...
def piirra_koppen(koppen, landmask):
    ...
    landmask : 2D numpy-taulukko
        Maamaski, jossa 1 = maa ja 0 = meri.
    """

    # ------------------------------------------------------------
    # Köppen–Geiger-väripaletti
    # ------------------------------------------------------------
    # Värit vastaavat yleisesti käytettyä Köppen–Geiger-palettia.
    # Af = sininen trooppinen sademetsäilmasto.
    # ------------------------------------------------------------

    varit = {
        # A: Trooppiset ilmastot
        "Af":  "#0000FE",   # Trooppinen sademetsä
        "Am":  "#0078FE",   # Trooppinen monsuuni
        "Aw":  "#46A9FA",   # Trooppinen savanni
        "As":  "#46A9FA",   # Trooppinen kesäkuiva savanni

        # B: Kuivat ilmastot
        "BWh": "#FF0000",   # Kuuma aavikko
        "BWk": "#FF9696",   # Kylmä aavikko
        "BSh": "#F5A300",   # Kuuma puoliaavikko
        "BSk": "#FFDC64",   # Kylmä puoliaavikko
    ...
    # Maskataan meri
    # ------------------------------------------------------------
    # Merialueet (landmask == 0) jätetään piirtämättä.
    # Tämä on turvallisempi tapa kuin numero * landmask2,
    # koska numero-taulukko pysyy kokonaislukuna.
    # ------------------------------------------------------------
    landmask2=np.copy(landmask)
    landmask2=np.where(landmask2==0, np.nan, landmask2)
    numero_masked = np.ma.masked_where(
        landmask == 0,
        numero
    )
    ...
    for luokka in luokat:
        ...
        # Lisätään legenda vain jos luokka esiintyy datassa
...
def max_likelihood_raster_turbo(
    rasteri_lista,
    hae_lista,
    sigma_lista,
    weight_lista,
    ...
    """
    Etsii rastereista pikselit, jotka parhaiten vastaavat
    annettua ympäristöparametrien joukkoa.

    Parametrien SYÖTTÖJÄRJESTYS määrittää niiden prioriteetin:
    ensimmäinen parametri vaikuttaa eniten, toinen seuraavaksi jne.

    weight_lista säätää käyttäjän haluamaa vaikutuksen voimakkuutta.

    type_lista:
        "target"  = arvo pyritään saamaan mahdollisimman lähelle targetia
        "minimum" = target on alaraja, suurempi arvo on parempi
        "maximum" = target on yläraja, pienempi arvo on parempi
    ...
    if priority_decay <= 0:
        raise ValueError("priority_decay pitää olla > 0.")

    if penalty_power <= 0:
        raise ValueError("penalty_power pitää olla > 0.")
    ...
    # Oletuksena kaikki ovat target-tyyppisiä
    if type_lista is None:
        type_lista = ["target"] * len(rasteri_lista)

    if len(type_lista) != len(rasteri_lista):
        raise ValueError(
            "type_lista pitää olla saman pituinen kuin rasteri_lista."
        )

    valid_types = {"target", "minimum", "maximum"}

    if any(t not in valid_types for t in type_lista):
        raise ValueError(
            'type_lista saa sisältää vain: '
    ...
    # Prioriteetit
    #
    # Ensimmäinen parametri = 1.0
    # Toinen = priority_decay
    # Kolmas = priority_decay^2
    # jne.
    # ---------------------------------------------------------

    priority = priority_decay ** np.arange(
        len(rasteri_lista),
        dtype=np.float64
    )
    ...
    for i, (
        ...
        # Puuttuva arvo tekee pikselistä invalidin
        ...
        elif param_type == "minimum":

            # Targetin alapuolella rangaistaan.
            # Targetin yläpuolella ei rangaista.
            z = np.maximum(
                (target - raster) / sigma,
                0.0
            )

        elif param_type == "maximum":

            # Targetin yläpuolella rangaistaan.
            # Targetin alapuolella ei rangaista.
            z = np.maximum(
                (raster - target) / sigma,
                0.0
            )
        ...
        # -----------------------------------------------------
        # Epälineaarinen penalty
        # -----------------------------------------------------

        penalty = z ** penalty_power

        # -----------------------------------------------------
        # Järjestys + käyttäjän paino
        # -----------------------------------------------------
    ...
    if not np.any(valid):
        raise ValueError(
            "Rastereissa ei ole yhtään validia pikseliä."
        )
...
def max_likelihood_raster_rocket(
    rasteri_lista,
    hae_lista,
    sigma_lista,
    weight_lista,
    ...
    """
    Moniparametrinen rasterihaku.

    Parametrien järjestys määrää prioriteetin:
        1. parametri = tärkein
        2. parametri = seuraavaksi tärkein
        ...

    weight_lista säätää käyttäjän antamaa vaikutusta.

    type_lista:
        "target"  = mahdollisimman lähelle tavoitetta
        "minimum" = target on alaraja
        "maximum" = target on yläraja

    Palauttaa:
        probability_surface
            Kokonaislikelihood 0...1

        max_likelihood_pixels
            TOP N pikselien (row, col)

        max_likelihood_coords
            TOP N pikselien [[lon, lat], ...]

        parameter_surfaces
            Jokaisen parametrin oma vaikutuspinta 0...1

        parameter_scores
            Parametrien log-score-pinnat ennen normalisointia

        effective_weights
            Parametrien lopulliset painot
    """
    ...
    if n_params == 0:
        raise ValueError(
            "rasteri_lista ei voi olla tyhjä."
        )

    if any(s <= 0 for s in sigma_lista):
        raise ValueError(
            "sigma-arvojen pitää olla > 0."
        )

    if any(w < 0 or w > 1 for w in weight_lista):
        raise ValueError(
            "Painojen pitää olla välillä 0...1."
        )

    if top_n < 1:
        raise ValueError(
            "top_n pitää olla vähintään 1."
        )

    if priority_decay <= 0:
        raise ValueError(
            "priority_decay pitää olla > 0."
        )

    if penalty_power <= 0:
        raise ValueError(
            "penalty_power pitää olla > 0."
        )
    ...
    # Oletuksena kaikki ovat target-tyyppisiä
    if type_lista is None:
        type_lista = ["target"] * n_params

    if len(type_lista) != n_params:
        raise ValueError(
            "type_lista pitää olla saman pituinen "
            "kuin rasteri_lista."
        )
    ...
    total_log_score = np.zeros(
        shape,
        dtype=np.float64
    )

    # Jokaisen parametrin oma log-score
    parameter_scores = []

    # Jokaisen parametrin oma likelihood
    parameter_surfaces = []
    ...
    for i, (
        ...
        # Parametrin oma score
        parameter_score = np.full(
            shape,
            np.nan,
            dtype=np.float64
        )

        parameter_score[valid] = (
            -effective_weights[i]
            * penalty[valid]
        )
        ...
        # -----------------------------------------------------
        # Parametrin oma likelihood
        #
        # Paras arvo = 1
        # -----------------------------------------------------

        parameter_surface = np.full(
            shape,
            np.nan,
            dtype=np.float64
        )
        ...
        if np.any(valid):
            ...
            best_parameter_score = np.max(
                parameter_score[valid]
            )

            parameter_surface[valid] = np.exp(
                parameter_score[valid]
                - best_parameter_score
            )
        ...
        total_log_score[valid] += (
            parameter_score[valid]
        )

        total_log_score[~valid] = np.nan
    ...
    best_flat = best_flat[
        np.argsort(
            total_log_score.flat[best_flat]
        )[::-1]
    ]
...
def calculate_culture_static():
    ...
    roma_list=[relief*suurin_manner_mask,
    ...
    #bounds=(-180.0, -90.0, 180.0, 90.0),
    #top_n=10) 
    roma_surface, roma_pixels, roma_coords = max_likelihood_raster_turbo(
    rasteri_lista=[
        relief*suurin_manner_mask,
        distsea*suurin_manner_mask,
        bio6*suurin_manner_mask,
        bio5*suurin_manner_mask,
        bio12*suurin_manner_mask,
    ],
    ...
    # Kuinka nopeasti parametrien prioriteetti laskee
    priority_decay=0.5,

    # 2 = normaali, 4 = paljon jyrkempi
    penalty_power=2,
   )
    ...
    (paris_probability_surface,paris_pixels,
    paris_coords,
    paris_parameter_surfaces,
    paris_parameter_scores,
    paris_effective_weights,
    ) = max_likelihood_raster_rocket(

    rasteri_lista=[
        relief*suurin_manner_mask,
        distwater*suurin_manner_mask,
        bio6*suurin_manner_mask,
        bio5*suurin_manner_mask,
        bio12*suurin_manner_mask,
    ],
    ...
    palku_x=paris_pixels[0,1]
    palku_y=paris_pixels[0,0]
    palku_lon=paris_coords[0,1]
    palku_lat=paris_coords[0,0]
    ...
    result = calculate_pastoral_nomadism(
    precips12,
    temps12,
    relief*1000,
    planet_radius*1000,
    ...
    tilt=tilt,
    mvelp=mvelp,
    orbital_period_in_years=orbital_period_years,
    rotation_period_in_hours=rotation_period_hours,
    )
    PNI = result["PNI"]
    PAR12 = result["PAR12"]
    ANPP = result["ANPP_kg_ha"]
    forage = result["usable_forage_kg_ha"]
    camel_lsu = result["camel_lsu_per_pixel"]
    sheep_lsu = result["sheep_goat_lsu_per_pixel"]
    cattle_lsu = result["cattle_lsu_per_pixel"]
    camel_pop = result["camel_population"]
    sheep_pop = result["sheep_goat_population"]
    cattle_pop = result["cattle_population"]
    camel_density = result["camel_density"]
    sheep_density = result["sheep_goat_density"]
    cattle_density = result["cattle_density"]

    
    #plt.imshow(camel_density*landmask)
    #plt.show() 
    ...
    plt.scatter(x=paris_coords[0,0],y=paris_coords[0,1], marker="s", edgecolors="k", linewidths=1,  color="purple", s=60) 
    plt.scatter(x=paris_coords[:,0],y=paris_coords[:,1], marker="s",edgecolors="k", linewidths=1,  color="purple", s=10) 
    plt.text(paris_coords[0,0],paris_coords[0,1],"Paris", color="purple", fontsize=16 ) 
    #plt.contour(pastoral_density*landmask, levels=[0.01, 0.1], colors=["orange"], lw=2, alpha=1, origin='upper', extent=[-180,180,-90,90],)
    #plt.contour(camel_density*landmask, levels=[30], colors=["orange"], lw=2, alpha=1, origin='upper', extent=[-180,180,-90,90],)
...
if __name__ == "__main__":
    ...
    if(generate_dem==1):
        ...
        dem0=fractal_height_modifier(imagee, p=2.5, a=3.0, lam=0.3)        
        ...
        #imagee=normalize(imagee)
        #imagee2=normalize(imagee2)
        #imagee=imagee*0.7+0.3*imagee2
         
        ##dem0=normalize(np.pow(normalize(np.exp(np.exp(imagee)*imagee2)), 2.5)) 
    ...
    #quit(-1)   
    temps12, precips12, winds12_x, winds12_y, winds12_z, ilmasto, planeetta = (
    laske_planeetan_ilmasto(
        dem=dem,
        relief=relief,
        landmask=landmask,
    ...
    temp_annual=bio1
    precip_annual=bio12
    planet_meant = np.average(temp_annual, weights=painom_matriisi) 
    planet_meanp = np.average(precip_annual, weights=painom_matriisi)
    print("T_mean C   ",planet_meant )        
    print("P_mean mm/a",planet_meanp )  
    plt.imshow(temp_annual)
    plt.show()
    ...
    biomi_kartta = luokittele_biomit(relief, temp_annual, precip_annual, sealevel=0.5)
   # 3. Kutsutaan aluohjelmaa laskentaa varten
    luokat = luokittele_holdridge(
    temp_annual,
    precip_annual,
    nodata_arvo=np.nan,
    )

    piirra_holdridge_vyohykkeet(luokat*landmask)
    ...
    piirra_koppen(koppen, landmask) 

    #plt.imshow(temp_annual, cmap="coolwarm", extent=[-180,180,-90,90], origin="upper")  
    #cont1=plt.contour(temp_annual, levels=[-40,-30,-20,-10,0,10,20,30,40], extent=[-180,180,-90,90], origin="upper") 
    #plt.clabel(cont1)
    #plt.imshow(precip_annual)   
    #plt.show()
    #quit(-1)
