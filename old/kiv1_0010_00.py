...
def laske_perus_ilmasto(relief, tilt=23.5, ecc=0.0167, mvelp=102.9, P=1.0):
    """
    ...
    mvelp : float
        ...
        Käytetään vuodenaikojen ja etäisyyden hienosäätöön.
    ...
    kk_temp = np.zeros((12, height, width))
    kk_tuuli = np.zeros((12, height, width))
    kk_sade = np.zeros((12, height, width))

    # ============================================================
    # MAASTON KALTEVUUS
    # ============================================================

    # Kaltevuus X-suunnassa.
    # Positiivinen = korkeus kasvaa oikealle.
    rinne_x = np.gradient(relief, axis=1)
    ...
    for kk in range(12):
        ...
        # kk = 0  -> vuoden alku
        # kk = 3  -> kevät/kesä
        # kk = 6  -> vuoden puoliväli
        # kk = 9  -> syksy/talvi
        #
        # Tämä antaa auringon liikkua pohjoisen ja etelän välillä.
        ...
        # Auringon näennäinen leveysaste.
        #
        # Maa:
        # noin +23.5° kesällä
        # noin 0° päiväntasauksessa
        # noin -23.5° talvella

        aurinko_leveysaste = np.degrees(
            tilt_rad * np.sin(vuoden_kulma)
        )

        # Normalisoitu sijainti -1 ... +1
        aurinko_y = aurinko_leveysaste / 90.0

        # --------------------------------------------------------
        # RADAN SOIKEUS
        # --------------------------------------------------------

        # Maapallon etäisyys auringosta vaihtelee hieman.
        #
        # Perusmallissa etäisyys:
        # 1 = keskimääräinen etäisyys
        #
        # ecc kasvattaa/pienentää vaihtelua.
        ...
        # Lähempänä aurinkoa = enemmän energiaa.

        aurinko_voima = 1.0 / (aurinkoetaisyys ** 2)
        ...
        # --------------------------------------------------------
        # 1. Leveysasteen perusvaikutus
        # --------------------------------------------------------

        # Päiväntasaaja lämmin.
        # Navat kylmiä.

        leveysasteen_lampo = (
            30.0
            - 0.32 * np.abs(latitude)
        )

        # --------------------------------------------------------
        # 2. Auringon sijainnin vaikutus
        # --------------------------------------------------------

        # Kuinka kaukana paikka on auringon alla olevasta
        # leveysasteesta.

        etaisyys_auringosta = np.abs(
            latitude - aurinko_leveysaste
        )

        # Mitä lähempänä aurinkoa, sitä lämpimämpää.
        #
        # Tämä tekee tropiikista vuodenaikojen mukaan liikkuvan.

        aurinko_lampo = (
            10.0
            * np.exp(
                -(etaisyys_auringosta / 35.0) ** 2
            )
        )

        # --------------------------------------------------------
        # 3. Vuodenaikojen vaikutus
        # --------------------------------------------------------
        ...
        # --------------------------------------------------------
        # 4. Radasta tuleva lämpövaikutus
        # --------------------------------------------------------
        ...
        # Keskimäärin noin 6.5 °C / kilometri.
        ...
        # Tuulivyöhykkeet liikkuvat hieman auringon mukana.

        # Auringon sijainnin vaikutus vyöhykkeisiin.
        #
        # 0 = päiväntasaaja
        # 1 = napa
        ...
        tuuli = np.zeros_like(Y)

        # --------------------------------------------------------
        # Hadley-solut / tropiikki
        # --------------------------------------------------------

        tropiikki = (
            abs_siirretty_y < 0.30
        )
        ...
        keskileveydet = (
            (abs_siirretty_y >= 0.30)
            &
            (abs_siirretty_y < 0.65)
        )

        tuuli[keskileveydet] = 1.0
        ...
        # Noin 45-60° leveysasteilla on toinen sadehuippu.
        ...
        # Kylmä ilma pystyy sisältämään vähemmän kosteutta.
        ...
        # Lämmin ilma -> enemmän sadetta.
        # Kylmä ilma -> vähemmän.

        kosteus = np.clip(
            (kk_temp[kk] + 15.0) / 30.0,
            0.05,
            2.0
        )
        ...
        # Jos tuuli puhaltaa kohti rinnettä,
        # kostea ilma nousee ja syntyy sadetta.
        ...
        # P skaalaa planeetan vuoden pituuden vaikutusta.
    ...
    return kk_temp, kk_tuuli, kk_sade
...
if __name__ == "__main__":
    ...
    tuuli_x, tuuli_y, tuuli_z=laske_tuuli(dem, kiertosolut=3)
   ...
   # 3. Kutsutaan aluohjelmaa laskentaa varten
    ...
    print(f"Kulttuurin alkupaikka pikseli ({kulttuuri_x}, {kulttuuri_y}) -> K Longitude: {kulttuuri_lon:.4f}, K Latitude: {kulttuuri_lat:.4f}")    
    ...
    plt.text(kulttuuri_lon, kulttuuri_lat, "Kult", fontsize=16) 
