...
def load_many_ratsers_numpy_taulukkoon(maara,tiedostopolut, new_width, new_height, menetelma=Resampling.bilinear):
    """
...
def parse_args():
    ## command line args
    global seed1, width, height, dem_max, dem_min
    ...
    parser.add_argument(
    "--peak", type=int, default=dem_max, help="Highest mountain meters"
    )
    parser.add_argument(
    "--abyss", type=int, default=dem_min, help="Lowest depth meters"
    )
...
def laske_kulttuuri_staattinen():
    ...
    aly_todennakoisyyskartta = laske_synnyinsija_todennakoisyys(relief*suurin_manner, temp_annual, precip_annual)
    ...
    habitability=laske_human_habitability_index_annual(temp_annual, precip_annual)
    ## mureybat
    target_alt = 250.0
    target_temp = 17
    target_rain = 250.0
    ...
    kulttuuri_tod,agriculture_index,pastoral_index,suitability,kulttuuri,leviamisaika,vaesto,carrying_capacity=etsi_paikka_ja_levia(target_alt, target_temp, target_rain,relief*suurin_manner, temp_annual*suurin_manner, precip_annual* suurin_manner, ajo_vuodet=10000)
    kulttuuri_idx = np.unravel_index(np.argmax(kulttuuri_tod*suurin_manner), kulttuuri_tod.shape)
    ...
    kulttuuri2_tod,agriculture_index2,pastoral_index2,suitability2,kulttuuri2,leviamisaika2,vaesto2,carrying_capacity2=etsi_paikka_ja_levia(target2_alt, target2_temp, target2_rain,relief*suurin_manner, temp_annual, precip_annual, ajo_vuodet=1000)
    kulttuuri2_idx = np.unravel_index(np.argmax(kulttuuri2_tod*suurin_manner), kulttuuri2_tod.shape)
    ...
    #plt.show()    
    civ_raster= suurin_manner*laske_sivilisaatiopisteet(precip_annual, temp_annual, distrivers, distsea, distmountains, relief*suurin_manner)# --- ESIMERKKIKÄYTTÖ ---
...
if __name__ == "__main__":
   ...
   # 3. Kutsutaan aluohjelmaa laskentaa varten
    ...
    print(f"Alueen painotettu keskilämpötila: {planeetan_keski_lampotila:.2f} °C")
    print(f"Alueen painotettu keskisademäärä: {planeetan_keski_sademaara:.2f} mm")
