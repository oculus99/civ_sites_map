...
MAA_GHG_DELTA_T_C = 53.0


# CO2:n säteilypakotteen kerroin.
CO2_FORCING_COEFF = 5.35

# Maan CO2-vertailupitoisuus.
...
# Nykyisessä mallissa nopea/hidas pyöriminen vaikuttaa
# globaaliin lämpötilaan vain maltillisesti.
...
def laske_planeetan_parametrit(
    ...
    """
    Muodostaa planeetan parametripaketin.
    ...
    S1_Wm2
        Tähdeltä planeetan radalle saapuva säteily W/m².

    star_teff_K
        Tähden efektiivinen lämpötila Kelvininä.

    orbital_period_years
        Planeetan kiertoaika vuosina.

    rotation_period_hours
        Planeetan pyörimisaika tunteina.
    ...
    mvelp
        Perihelin pituusaste.

    atmosphere_pressure
        Ilmakehän paine Maan ilmakehinä.

    aerosol_coeff
        Tähden säteilystä jäljelle jäävä osuus.
    ...
    ----------
    dict
        Planeetan alkuperäiset ja johdetut parametrit.
    """
    ...
    #
    # Maa:
    # 24 h -> noin 3 solua / pallonpuolisko
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Aerosolien jälkeen jäljelle jäävä säteily
    # --------------------------------------------------------
...
def laske_planeetan_sateily(
    ...
    """
    Laskee planeetan säteilytasapainon.

    Kasvihuoneilmiö kalibroidaan Maan avulla,
    mutta sitä voidaan käyttää myös muilla planeetoilla.

    CO2:n vaikutus tulee erillisenä säteilypakotteena.
    """
    ...
    if vesihoyry_kerroin < 0:
        raise ValueError(
            "vesihoyry_kerroin pitää olla >= 0."
        )

    if kasvihuone_kerroin < 0:
        raise ValueError(
            "kasvihuone_kerroin pitää olla >= 0."
        )

    if ilmastosensitiivisyys < 0:
        raise ValueError(
            "ilmastosensitiivisyys pitää olla >= 0."
        )

    # --------------------------------------------------------
    # 1. Tähdeltä tuleva säteily
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 3. Planeetan absorboima säteily
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 4. Eksentrisyyden vuosikeskiarvo
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 8. ILMAKEHÄLLINEN REFERENSSILÄMPÖTILA
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 9. ULOSMENEVA PITKÄAALTOINEN SÄTEILY
    # --------------------------------------------------------
    ...
    #
    # Huom:
    # Tämä ei ole tässä yksinkertaisessa mallissa
    # täysin suljettu säteilytasapaino, koska kasvihuone
    # lisätään lämpötilakorjauksena.
    #
    # Arvo säilytetään diagnostista käyttöä varten.
    # --------------------------------------------------------
...
def laske_planeetan_lampotila(
    ...
    """
    Laskee planeetan globaalin ja alueellisen lämpötilan.

    Tärkeä muutos:
    Maan lämpötilaan ei lisätä enää erillistä +26 °C
    kalibrointia.

    Kasvihuoneilmiö käsitellään jo säteilyfunktiossa.
    """
    ...
    # ========================================================
    # GLOBAALI REFERENSSILÄMPÖTILA
    #
    # EI enää +26 °C kalibrointia.
    # ========================================================
...
def laske_perus_ilmasto(
    ...
    Muutokset:
    - ei enää -8 °C lopullista keinotekoista kalibrointia
    - sadanta ei saa ylimääräistä ×0.5-leikkausta
    - vuoden pituus ei suoraan muuta sadannan määrää
    """
    ...
    for kk in range(12):
        ...
        # MERI
        #
        # Ei tehdä merestä globaalia kerrointa.
        # Merivaikutus on jo paikallinen.
        # ====================================================
        ...
        # ====================================================
        # EI KERROTA SADANTAA VUODEN PITUUDELLA
        #
        # Tämä on tärkeä muutos.
        #
        # Orbitaalinen vuosi vaikuttaa kausirytmiin,
        # ei suoraan veden määrään.
        # ========================================================

        # Ei:
        #
        # sade_pohja *= orbital_period_years
        #
        # ========================================================
        ...
        # TALLENNA SADE
        #
        # Ei enää ylimääräistä ×0.5-leikkausta.
        # ====================================================
    ...
    # ========================================================
    # EI ENÄÄ -8 °C KALIBROINTIA
    # ========================================================

    # kk_temp pysyy sellaisenaan.
    kk_temp= kk_temp+2.5
    kk_sade=kk_sade*1.5
    return (
        kk_temp,
...
def laske_planeetan_ilmasto(
    ...
    # ========================================================
    # LOPULLINEN SADEKORJAUS
    #
    # Ei enää:
    #
    #   meriosuus × 3.5
    #
    # koska merivaikutus käsitellään jo paikallisesti.
    #
    # Lämpötila saa vaikuttaa sadantaan vain maltillisesti.
    # ========================================================
    ...
    sademaaran_lampo_kerroin = np.clip(
        1.0
        +
        0.015
        *
...
def laske_miami_npp_teff_correction(star_teff, T_sun=5772.0):
    """
    Laskee tähden PAR-kertoimen suhteessa Aurinkoon
    tähden efektiivisen lämpötilan perusteella.
    ...
    T_sun : float
        Auringon efektiivinen lämpötila [K]

    Returns
    -------
    f_par : float
        PAR-säteilytehon suhde Aurinkoon.
    """

    # Planckin lain vakiot
    h = 6.62607015e-34       # Planck [J s]
    c = 299792458.0          # valon nopeus [m/s]
    ...
    def par(teff):
        ...
        return np.trapezoid(B, wavelength)

    par_star = par(star_teff)
    par_sun = par(T_sun)

    return par_star / par_sun



def laske_temperature_corrected_npp_from_miami(T, npp_miami):
    T_min = 0
    T_opt = 20
    T_max = 60
...
def calculate_culture_static():
    ...
    #plt.show()
    intelligence_surface, intelligence_pixels, intelligence_coords = max_likelihood_raster_turbo(
    rasteri_lista=[
...
if __name__ == "__main__":
   ...
   # 3. Kutsutaan aluohjelmaa laskentaa varten
    ...
    rotation_period_in_hours=rotation_period_hours)
    npp_0 = laske_npp_miami(temp_annual, precip_annual, nodata_arvo=np.nan)*landmask 
    npp_teffcoeff=laske_miami_npp_teff_correction(star_teff, T_sun=5772.0)
    npp=laske_temperature_corrected_npp_from_miami(temp_annual, npp_0)*npp_teffcoeff
